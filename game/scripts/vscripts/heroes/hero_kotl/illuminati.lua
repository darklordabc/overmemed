...
function IlluminateEnd( keys )
	...
	ability.illuminate_heal = 0
	local damage_per_second = ability:GetLevelSpecialValueFor("damage_per_second", (ability:GetLevel() - 1))
	local heal_per_second = ability:GetLevelSpecialValueFor("heal_per_second", (ability:GetLevel() - 1))
	...
	ability.illuminate_heal = ability.illuminate_channel_time * heal_per_second
...
function IlluminateProjectileHit( keys )
	...
	if time < 0.5 then
		...
		if target:GetTeamNumber() == DOTA_TEAM_GOODGUYS then
			target:Heal(ability.illuminate_heal, caster)
		end
	end
end
